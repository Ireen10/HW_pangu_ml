...
```json
{
  ...
  "image": {
    "type": "relative_path",
    "format": "<image/jpeg | image/png 等，须与 tar 内实际编码一致>",
    "relative_path": "<对应分片 tar 包内的相对路径，扩展名与编码一致，如 .jpg / .png>",
    "width": <整数>,
    "height": <整数>
  }
}
...
- `format` 对应该图像编码；常见为 `image/jpeg` 或 `image/png`（例如源为 PNG 时保留 PNG，其它格式多为 JPEG）。
      ...
      "content": [
        {
          ...
          "image": {
            "type": "relative_path",
            "format": "image/png",
            "relative_path": "example/subdir/frame_001.png",
            "width": 640,
            "height": 426
          }
        },
